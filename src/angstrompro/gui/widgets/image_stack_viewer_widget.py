...
class QtMatplotCanvas(FigureCanvas):
    mouseMoveEventSignal    = Signal(QtCore.QEvent)
    mousePressEventSignal   = Signal(QtCore.QEvent)
    mouseReleaseEventSignal = Signal(QtCore.QEvent)
...
class ImageStackViewerWidget(QtWidgets.QWidget):
    ...
    def __init__(self, *args, **kwargs):
        super(ImageStackViewerWidget, self).__init__(*args, **kwargs)
    ...
    def setup_palette(self, cmap_list: list) -> None:
        """Load colormap palette from config (list of colormap name strings)."""
    ...
    def initUiMembers(self):
        ...
        # Seed palette combobox with a default so set_colormap() is always safe to call
    ...
    def initNonUiMembers(self):
        ...
        self.img_color_map = 'gray'   # default; overwritten by set_colormap() once palette is loaded
    ...
    def set_colormap(self):
        ...
        if self.img_is_rt_cmp_on:
            ...
            if color_map_name in matplotlib.colormaps:
